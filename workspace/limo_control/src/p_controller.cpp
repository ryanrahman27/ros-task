...
public:
    PController() : Node("p_controller")
    {
        ...
        RCLCPP_INFO(this->get_logger(), "P Controller initialized");
        RCLCPP_INFO(this->get_logger(), "Goal: (%.2f, %.2f)", goal_x_, goal_y_);
        RCLCPP_INFO(this->get_logger(), "Kp_linear: %.2f, Kp_angular: %.2f", kp_linear_, kp_angular_);
    }
...
private:
    ...
    void controlLoop()
    {
        ...
        double linear_vel = kp_linear_ * distance_error;
        ...
        RCLCPP_DEBUG(this->get_logger(), 
            "Pos: (%.2f, %.2f), Goal: (%.2f, %.2f), Dist: %.3f, Angle: %.3f, Vel: (%.3f, %.3f)",
            current_x_, current_y_, goal_x_, goal_y_, distance_error, angle_error, linear_vel, angular_vel);
    }
